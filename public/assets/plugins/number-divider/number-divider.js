...
(function ($) {
	...
	$.fn.divide = function (params) {
		...
		function getDivided(text) {
			...
			if (!isNumeric(text) && text.length > 0 || text == '') {
				return -1;
			}
		...
		this.each(function () {
			...
			$this.bind("input.divide change.divide", function () {
				...
				if (!data) {
					...
					if (name != undefined) {
						...
						if (options.array == true) {
							$this.parent().append("<input type='hidden' class='" + name + "' name='" + name + "[]'>");
						} else {
							$this.parent().append("<input type='hidden' class='" + name + "' name='" + name + "'>");
						}
						data.name = name;
				...
				if (data.name) {
					if (result <= 0) {
						// value <= 0
						$this.siblings("." + data.name).val(0);
					} else {
						$this.siblings("." + data.name).val(text * 1);
					}
				}
			});
